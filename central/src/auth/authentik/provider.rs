...
use crate::{auth::config::FlowPropertymapping, CLIENT};

use super::{get_uuid, AuthentikConfig};



pub async fn generate_provider_values(name: &str, oidc_client_config: &OIDCConfig, secret: &str, conf: &AuthentikConfig, token: &str) -> anyhow::Result<Value> {
...
pub async fn get_provider(
    ...
    let pk = get_uuid(&provider_url, conf, token, &id).await.expect(&format!("Property: {:?}", id));
    CLIENT
        .get(&format!(
...
pub async fn compare_provider(
    ...
    let wanted_client = generate_provider_values(name, oidc_client_config, secret, conf, token).await?;
    Ok(client.get("client_secret") == wanted_client.get("client_secret")
